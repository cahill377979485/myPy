...
if __name__ == '__main__':
    ...
    listBean = []
    i = 1
    for item in soup.findAll('div', class_='c-single-text-ellipsis'):
        listNum.append(i)
        i = i + 1
    for item in soup.findAll('div', class_='c-single-text-ellipsis'):
        listTitle.append(item.get_text())
    for item in soup.findAll('div', class_=re.compile('hot-desc_1m_jR small_Uvkd3( ellipsis_DupbZ)?')):
