...
# -*- coding:utf-8 -*-
# @Time : 2021/4/9 14:37
# @File : 冰雹猜想由总值遍历寻找合适的n使max最小.py
# @Author:
# !/usr/bin/env python3
# -*- coding:utf-8 -*-
# @Time : 2021/4/9 14:20
# @File : 冰雹猜想由总值遍历寻找合适的n.py
# @Author:
    ...
    while flag:
        ...
        if n == 1:
            ...
            if theMinDelta == 0:
                ...
                theMinDelta = delta
                resultDelta = '关于最接近目标总和的结论：目标总和为%d n=%d的冰雹数据中，总和是%d 最小值%d 最大值%d 差值为%d' % (
                    target, x, my_sum, my_min, my_max, delta)
            if theMinMax == 0:
                theMinMax = my_max
            elif my_max < theMinMax:
                theMinMax = my_max
                resultMax = '关于冰雹数据中最大值最小的结论：目标总和为%d n=%d的冰雹数据中，总和是%d 最小值%d 最大值%d' % (
                    target, x, my_sum, my_min, my_max)
        elif n < 1:
            print("已完成n=%d" % x)
